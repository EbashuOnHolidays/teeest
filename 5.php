...
function invertTree(BinaryNode $object)
{
    $leftTemp = $object->left;
    $rightTemp = $object->right;

    $object->left = $rightTemp;
    $object->right = $leftTemp;

    if ($object->left) {
        invertTree($object->left);
    }

    if ($object->left) {
        invertTree($object->right);
    }
}
...
class InvertBinaryTreeTest extends \PHPUnit_Framework_TestCase
{
    public function testInvert()
    {
        ...
        $invertedTree = clone $root;
        invertTree($invertedTree);
